...
class PDFCrossRefSection(PDFObjectLogger):
    def __init__(self):
...
class PDFCrossRefSubSection(PDFObjectLogger):
    ...
    def setEntry(self, objectId, newEntry):
        ...
        if numEntry is not None:
            ...
            errorMessage = 'Entry not found with id {id}'.format(id=objectId)
            self.log.error(errorMessage)
            return (-1, errorMessage)
...
class PDFCrossRefEntry(PDFObjectLogger):
    def __init__(self, firstValue, secondValue, entryType, offset=0):
        PDFObjectLogger.__init__(self)
        self.offset = offset
        ...
        if self.entryType == 'f' or self.entryType == 0:
            ...
            errorMessage = 'Unknown type "{}"'.format(entryType)
            self.log.error(errorMessage)
            if not forceMode:
                return (-1, errorMessage)
        ...
        if vtJsonDict.has_key('response_code'):
            if vtJsonDict['response_code'] == 1:
                if vtJsonDict.has_key('positives') and vtJsonDict.has_key('total'):
                    ...
                    self.log.error('Missing elements in the response from VirusTotal!!')
                if vtJsonDict.has_key('permalink'):
                    self.setDetectionReport(vtJsonDict['permalink'])
            else:
                self.setDetectionRate(None)
        else:
            self.log.error('Bad response from VirusTotal!!')
                                    ...
                                    if "/Length" in cryptFilterDict and cfmValue != '/AESV3':
                                        ...
                                        if keyLength is not None and keyLength.getType() == 'integer':
                                            ...
                                            if keyLength % 8 != 0:
                                                ...
                                                self.log.error('Decryption error: Key length not valid!!')
                                            # Check if the length element contains bytes instead of bits as usual
                                            if keyLength < 40:
                                                keyLength *= 8
                                        else:
                                            keyLength = defaultKeyLength
                                            self.log.error('Decryption error: Bad format for /Length!!')
                                    else:
                                        keyLength = defaultKeyLength
    ...
    def getTrailer(self, version=None):
        if version is None:
            for i in range(self.updates, -1, -1):
                ...
                # self.log.error('Trailer not found in file')
                return None
        else:
            if version > self.updates or version < 0:
                # self.log.error('Bad version getting trailer')
                return None
    ...
    def getXrefSection(self, version=None):
        if version is None:
            for i in range(self.updates, -1, -1):
                ...
                # self.log.error('Xref section not found in file')
                return None
        else:
            if version > self.updates or version < 0:
                return None
                ...
                if delim[2] == 'dictionary':
                    ...
                    if streamFound:
                        ...
                        if ret[0] != -1:
                            ...
                            pdfObject.log.error('Closing delimiter not found in dictionary object')
                        break
                elif delim[2] == 'string':
                    ret = self.readUntilClosingDelim(content, delim)
                    if ret[0] != -1:
                        ...
                        pdfObject.log.error('Closing delimiter not found in string object')
                    break
                    ...
                    if ret[0] != -1:
                        ...
                        pdfObject.log.error('Closing delimiter not found in hexadecimal object')
                    break
                elif delim[2] == 'array':
                    ret = self.readUntilClosingDelim(content, delim)
                    if ret[0] != -1:
                        ...
                        pdfObject.log.error('Closing delimiter not found in array object')
                    break
                elif delim[2] == 'name':
